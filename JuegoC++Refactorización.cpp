// JuegoC++Refactorización.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
...
int main()
{
    ...
    while (heroIsAlive && (enemyIsAlive1 || enemyIsAlive2)) {
        ...
        enemyDamage = generateattack(0, 150);

        if (enemyChoosed == 1) {
            //ATACAMOS AL ENEMIGO 1
            enemyHP1 = heroAttackEnemy(enemyName1, enemyHP1, heroDamage);
            enemyIsAlive1 = checkEnemyStatus(enemyName1, enemyHP1);
        }
        else {
            //ATACAMOS AL ENEMIGO 2
            enemyHP2 = heroAttackEnemy(enemyName2, enemyHP2, heroDamage);
